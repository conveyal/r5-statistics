  ...
  render () {
    return <div>
      ...
      <canvas
        ...
        onMouseMove={(e) => this.drawCanvas(e.clientX, e.clientY)}
        // remove crosshairs on mouse out
        onMouseOut={(e) => this.drawCanvas()}
        style={{ cursor: 'none' }}
        />
  ...
  drawCanvas = (x, y) => {
    ...
    for (let gridx = 0; gridx < MINUTES; gridx++) {
      for (let gridy = 0; gridy < STEPS; gridy++) {
        ...
        let col = colorScale(grid[cell]).rgb()
    ...
    if (x !== undefined && y !== undefined) {
      ...
      let probability = grid[step * MINUTES + minute] / iterations
      let cumulativeProbability = 0

      for (let sumStep = 0; sumStep <= step; sumStep++) {
        cumulativeProbability += grid[sumStep * MINUTES + minute] / iterations
      }
